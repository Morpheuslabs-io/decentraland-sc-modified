...
import "../erc721/ERC721URIStorageEnumerable.sol";
  ...
  EstateStorage
{
  modifier canTransfer(uint256 estateId) {
    require(
      _isApprovedOrOwner(msg.sender, estateId),
  ...
  function getLANDsSize(address _owner) public view returns (uint256) {
    ...
    for (uint256 i; i < balance; i++) {
      uint256 estateId = tokenOfOwnerByIndex(_owner, i);
   ...
   */
  function setManyUpdateOperator(uint256[] memory _estateIds, address _operator)
    public
  {
    ...
    returns (bool)
  {
    ...
    return
      _isApprovedOrOwner(operator, estateId) ||
      updateOperator[estateId] == operator ||
