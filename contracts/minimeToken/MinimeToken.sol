...
contract MiniMeToken is Controlled {
  ...
  function doTransfer(
    ...
    if (isContract(controller)) {
      ...
      require(
        ITokenController(controller).onTransfer(_from, _to, _amount) == true
      );
    }
    ...
    returns (uint256)
  {
    ...
    uint256 minVal = 0;
    uint256 maxVal = checkpoints.length - 1;
    while (maxVal > minVal) {
      uint256 mid = (maxVal + minVal + 1) / 2;
    ...
    return checkpoints[minVal].value;
  }
